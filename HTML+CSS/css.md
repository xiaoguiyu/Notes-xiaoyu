...
| :--------: | :------------------------: | :--------------: | :----------: | :------------------: |
|  块级元素  |   一行只能放一个块级元素   |  可以设置宽高度  | 容器的 100%  |   可以包含任何标签   |
|  行内元素  |   一行可以放多个行内元素   |   不能设置宽高   | 本身内容宽度 | 容纳文本或者其他行内 |
...
当行高**小于盒子高度，则文字偏上**；当行高**大于盒子高度，文字偏下。**
...
![box-model](css.assets/yilgJO.gif)
...
1. 给其父元素添加 `text align: center`

### 1.8 box-sizing 属性解决方案
...
1. 脱离文档流的控制（浮）移动到指定位置（动 (俗称脱标)
   脱离文档流的盒子，会漂浮在文档流的盒  子上面，不占位置。
...
**浮动元素**不同，只会压住它下面标准流的盒子，但是**不会压住下面标准流盒子里面的文字（图片）**但是**绝对定位（固定定位）会压住下面标准流所有的内容。**
...
1. 标准流
   可以让盒子上下排列或者左右排列, **垂直的块级盒子显示**就**用标准流布局。**
2. 浮动
   可以让多个块级元素一行显示或者左右对齐盒子, 多个块级盒子**水平显示就用浮动布局。**
3. 定位
   定位最大的特点是有层叠的概念, 就是可以让多个盒子前后暑压来显示。如果元素**自由**在某个盒子内**移动就用定位布局**。
